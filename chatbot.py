...
df=pd.read_csv('/content/convert.csv',sep='\
t',names=['question','answer'])
...
tokens'].max()==df['encoder input tokens']]
...
variables=self.encoder.trainable_variables+self.decoder.trainable_vari
ables
...
self.encoder,self.decoder=self.build_inference_model(base_encoder,base
_decoder)
...
decoder_outputs,new_states=self.decoder([target_seq,states],training=F
alse)
...
chatbot.summary()
tf.keras.utils.plot_model(chatbot.encoder,to_file='encoder.png',show_s
hapes=True,show_layer_activations=True)
tf.keras.utils.plot_model(chatbot.decoder,to_file='decoder.png',show_s
hapes=True,show_layer_activations=True)
